...
##################################################################################################################################################################
##################################################################################################################################################################
EnsembleRCBN<-function(PathProbs,N,DIR,subDIR,InputFileName){
  ...
  #subDIR is the sub-path within the repository towards the desired genotype file
  #InputFileName is the name of the file containing the genotype matrix (without the ".dat")
  ...
  while (n<N){
    ...
    PathProbs<-TEMP
    PathProbR<-PathProbs
    dir.create(paste0(DIR,"/",subDIR,"/",InputFileName,"_n",n,"/R"),recursive=TRUE)
    FinalFileName<-paste0(DIR,"/",subDIR,"/",InputFileName,"_n",n,"/R/PathProbR.RData")
    save(PathProbR,file=FinalFileName)
